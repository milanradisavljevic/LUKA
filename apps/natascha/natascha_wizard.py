#!/usr/bin/env python3
"""
NATASCHA TUI – Wizard-Pattern mit rich + InquirerPy.
...
FIRST_RUN_MARKER = Path.home() / ".natascha_first_run_done"
...
def show_banner(console: Console) -> None:
    ...
    if _pyfiglet is not None:
        logo = _pyfiglet.figlet_format("NATASCHA", font="slant")
    else:
        logo = _ASCII_LOGO
...
def show_first_run_help(console: Console) -> None:
    console.print(
        Panel(
            f"[{STYLE_BRAND}]Willkommen bei NATASCHA![/{STYLE_BRAND}]\n\n"
...
def _test_api_connection(config: dict[str, Any], console: Console) -> None:
    ...
    if not api_cfg.get("enabled"):
        ...
        console.print(
            f"[{STYLE_DIM}]Setze [api] enabled = true in natascha_config.toml.[/{STYLE_DIM}]"
        )
        console.print()
        return
