...
def test_load_feedback_json_parses_fixture() -> None:
    ...
    assert data.schueler == "Tamara Ebner"
...
def test_process_file_creates_docx_and_can_be_reopened(tmp_path: Path) -> None:
    ...
    output_path = (
        paths.output_dir / "Kommentar -Krass, Digga!...- Matthias Bachler_feedback.docx"
    )
...
def test_summary_table_present_in_output() -> None:
    ...
    noten_table = next(
        (t for t in doc.tables if any("GESAMTNOTE" in c.text for r in t.rows for c in r.cells)),
        None,
    )
    assert noten_table is not None, "GESAMTNOTE-Tabelle nicht gefunden"
...
def test_header_contains_datei_without_config() -> None:
    ...
    assert "Tamara Ebner" in all_table_text
