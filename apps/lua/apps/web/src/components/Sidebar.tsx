...
import { Home,BookOpen,SpellCheck,Library,Settings,HelpCircle,ChevronLeft,ChevronRight,Plus,Bug,RefreshCw, type LucideIcon } from 'lucide-react';
import type { ActiveView } from '../lib/types';
import { WORK_AREAS,workArea } from '../lib/workNavigation';
import { FEATURES } from '../lib/features';
import { BrandSignature } from './BrandLogo';
import type { UseUpdaterReturn } from '../hooks/useUpdater';
/** An den Bereichsnamen gehängt, nicht an die Position: sonst verschiebt eine
 *  Umordnung der Bereiche stillschweigend die Icons. */
const icons: Record<string,LucideIcon>={Start:Home,Unterricht:BookOpen,Korrekturen:SpellCheck,Bibliothek:Library};
export function Sidebar({currentView,onViewChange,onNewDocument,onBugReport,updater}:{currentView:ActiveView;onViewChange:(view:ActiveView)=>void;onNewDocument:()=>void;onBugReport:()=>void;updater?:UseUpdaterReturn}){
 ...
 return <aside className={'luka-sidebar'+(collapsed?' is-collapsed':'')} aria-label="Hauptnavigation">
  ...
  <nav>{WORK_AREAS.map((area)=>{if(!FEATURES.natascha && ['korrektur','klassen'].includes(area.view))return null;const Icon=icons[area.label]??Home;return <button key={area.view} onClick={()=>onViewChange(area.view)} aria-label={area.label} title={area.label} aria-current={active===area?'page':undefined}><Icon size={20}/>{!collapsed && area.label}</button>;})}</nav>
   ...
   <div className="sidebar-version-row"><small>{collapsed ? version : 'LUKA '+version}</small>
    {tauri && updater &&
      <button className={updateNotified?'sidebar-update-btn has-update':'sidebar-update-btn'} onClick={()=>void updater.checkNow()} disabled={updater.state.phase==='checking'} aria-label="Auf Updates prüfen" title="Auf Updates prüfen">
        {/* Nur das Symbol: in 210 px passten Version und Beschriftung nicht
            nebeneinander, und die Versionsnummer wurde abgeschnitten. Der
            Klartext steht im Titel und im aria-label. */}
        <RefreshCw size={13} className={updater.state.phase==='checking'?'spin':undefined}/>
      </button>}
  </div></div>
 </aside>;
}
