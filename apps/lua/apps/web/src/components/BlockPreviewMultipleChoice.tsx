...
export function BlockPreviewMultipleChoice({ block, showSolution, solutionStep }: Props) {
  ...
  return (
    <div style={{ fontFamily: 'var(--font)', fontSize: '11pt', lineHeight: 1.6 }}>
      ...
      {fragen.map((frage: {
        ...
        const visible = isRevealed(frage.nr - 1);
              ...
              return (
                <label key={opt.key} style={{
                  ...
                  <span style={{
                    ...
                    background: visible && isCorrect ? 'var(--color-bg-selected)' : 'transparent',
                    fontSize: '9pt', fontWeight: 700, flexShrink: 0,
                  }}>
                    {visible && isCorrect ? <Check size={12} /> : ''}
                  </span>
                  <span style={{
                    color: visible && isCorrect ? '#000' : undefined,
                    fontWeight: visible && isCorrect ? 600 : 400,
                  }}>
                    {opt.key}: {opt.text}
                  </span>
                  {visible && isCorrect && (
